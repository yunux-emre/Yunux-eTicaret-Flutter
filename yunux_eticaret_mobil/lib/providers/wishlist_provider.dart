...
import 'package:mobil_projesi/models/wishlist_model.dart';
...
class WishlistProvider with ChangeNotifier {
  ...
  Future<void> addToWishlistFirebase({
    ...
    if (user == null) {
      MyAppFunctions.showErrorOrWarningDialog(
        context: context,
        subtitle: "Lütfen önce giriş yapın",
        fct: () {},
      );
      return;
    }
    ...
    try {
      await userstDb.doc(uid).set({
        'userWish': FieldValue.arrayUnion([
          {'wishlistId': wishlistId, 'productId': productId},
        ]),
      }, SetOptions(merge: true));

      Fluttertoast.showToast(msg: "Öğe eklendi");
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<void> removeWishlistItemFromFirestore({
    ...
    final User? user = _auth.currentUser;
    try {
      await userstDb.doc(user!.uid).set({
        'userWish': FieldValue.arrayRemove([
          {'wishlistId': wishlistId, 'productId': productId},
        ]),
      }, SetOptions(merge: true));
      _wishlistItems.remove(productId);
      Fluttertoast.showToast(msg: "Öğe kaldırıldı");
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<void> clearWishlistFromFirebase() async {
    ...
    try {
      ...
      _wishlistItems.clear();
      Fluttertoast.showToast(msg: "İstek listesi temizlendi");
    } catch (e) {
      rethrow;
    }
  }
